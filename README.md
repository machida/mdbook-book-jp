...
- `bin/audit.sh`
  - **導入状態の検査用。** 対象の mdBook プロジェクトが、現在のテーマ契約（`additional-css` の順序、`@import` 非依存、CSS ファイル同期）に沿っているかを確認します。
...
更新後に状態を確認したい場合は、次も実行してください。

```bash
bash ./bin/audit.sh /path/to/book-project
```

この監査では次を確認します。

- `additional-css` が `theme/mdbook-book-core.css` → `theme/mdbook-book-jp.css` で始まっていること
- 書籍側の `theme/mdbook-book-jp.css` に古い `@import` が残っていないこと
- 書籍側の `theme/mdbook-book-core.css` / `theme/mdbook-book-jp.css` が、`mdbook-book-jp` 本体と同期していること
...
- mdBook 標準 UI の固定配置やヒットエリア前提は尊重し、`.nav-chapters` のような標準ナビゲーションは色変更中心に留める

## テーマ編集の注意

`mdbook-book-jp` は mdBook 標準テーマの上に重ねる設計です。  
そのため、見た目を整えるときも **mdBook 側のレイアウト前提を壊さない** ことを優先します。

特に次は注意が必要です。

- `.nav-chapters`
  - 章の前後移動リンクは、見た目以上に広い固定ヒットエリアとして配置されています。常時背景・枠・影・大きな角丸を付けると、本文領域へ張り出して見えやすくなります。
- `.page-wrapper` / `.page` / `.content main`
  - 本文の中央寄せやサイドバー開閉時のオフセットは、mdBook 標準 CSS 側で計算されています。幅や余白の変更は、この前提を崩さない範囲に留める必要があります。
- `.table-wrapper`
  - mdBook 標準がすでに横スクロール用ラッパーを出すため、スクロール責務を `table` 本体へ移さない方が安全です。

要するに、**本文表現は積極的に調整してよいが、標準 UI は構造を変えずに装飾だけ変える**のが基本方針です。
